...
class MidiQuantizer:
    ...
    def quantize_piece(self, piece: MidiPiece) -> MidiPiece:
        ...
        df = self.quantize_record(df)

        out = MidiPiece(df=df, source=source)
        return out

    def quantize_record(self, record: dict | pd.DataFrame) -> dict:
        record["dstart_bin"] = np.digitize(record["dstart"], self.dstart_bin_edges) - 1
        record["duration_bin"] = np.digitize(record["duration"], self.duration_bin_edges) - 1
        record["velocity_bin"] = np.digitize(record["velocity"], self.velocity_bin_edges) - 1
